...
func populate_reward_display() -> void:
	...
	for reward_group: int in reward_data.keys():
		...
		for consumable_id in consumable_reward_ids:
			var draft_consumable_action_data: Array[Dictionary] = [
				{
			...
			var draft_consumable_action: BaseAction = ActionGenerator.create_actions(null, null, [], draft_consumable_action_data, null)[0]

			var consumable_reward_button: BaseRewardButton = Scenes.CONSUMABLE_REWARD_BUTTON.instantiate()
			consumable_reward_button.init(draft_consumable_action, reward_group)
...
func add_rewards(reward_group: int, money_amount: int, card_drafts: Array[Array], artifact_ids: Array[String], consumable_ids: Array[String], custom_action_data: Array[Array]):
	...
	# consumables
	var reward_consumable_ids: Array = reward_group_data.get("reward_consumable_ids", [])
